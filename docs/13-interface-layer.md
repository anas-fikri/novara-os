...
*   **`/add-mcp <name> <cmd> [args...]`** — Add a new MCP server to the configuration and connect to it immediately. If run without arguments, it launches a guided wizard with built-in templates (Filesystem, Git, Puppeteer, SQLite, Postgres, MSSQL, Fetch, Brave Search, Custom) supporting fully interactive connection parameter inputs.
